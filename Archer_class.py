        ...
        if now - self.last_update > self.frame_rate[self.mode]:
            ...
            if isinstance(self, Archer):
                if self.mode == 'attack01' and self.frame == 6:
                    Arrow(self.rect.center, self.current_target, 12.5, self.atk)
                elif self.mode == 'attack02' and self.frame == 10:
                    Arrow(self.rect.center, self.current_target, 20, self.super_atk)
                    self.kills = 0
...
class Knight(Unit):
    def __init__(self, coord):
        ...
        super().__init__(coord, animations, hp=100, atk=20, super_atk=40, frame_rate=frame_rate)

    def update(self):
        super().update()
        # Надо дописать логику


class Arrow(pygame.sprite.Sprite):
    def __init__(self, coord, target, v, damage):
    ...
    def update(self, *args, **kwargs):
        ...
        if pygame.sprite.collide_mask(self, self.target):
            self.target.lose_hp(self.damage)  # Наносим урон цели
